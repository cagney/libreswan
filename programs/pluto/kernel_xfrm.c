...
static void update_xfrm_acq_expires(struct logger *logger)
{
...
static void kernel_xfrm_init(struct logger *logger)
{
	...
	update_xfrm_acq_expires(logger);
...
{
	name_buf op_str;
	enum_short(&kernel_policy_op_names, op, &op_str);
	name_buf dir_str;
...
static const void *nlmsg_data(struct nlmsghdr *n, size_t size, struct logger *logger, where_t where)
{
	if (n->nlmsg_len < NLMSG_LENGTH(size)) {
		...
		llog(RC_LOG, logger,
		     ...
		     where->func,
		     str_sparse_long(&xfrm_type_names, n->nlmsg_type, &sb),
		     n->nlmsg_len, size);
		return NULL;
	}
...
static void process_addr_change(struct nlmsghdr *n, struct logger *logger)
{
	...
	ldbg(logger, "%s() xfrm netlink address change %s msg len %zu",
	     __func__, str_sparse_long(&rtm_type_names, n->nlmsg_type, &sb),
	     (size_t) n->nlmsg_len);
...
static void xfrm_kernel_sa_expire(struct nlmsghdr *n, struct logger *logger)
{
	...
	if (ue->hard) {
		if (impair.ignore_hard_expire) {
			llog(IMPAIR_STREAM, logger, "suppressing a HARD EXPIRE event");
			return;
		}
	} else {
		if (impair.ignore_soft_expire) {
			llog(IMPAIR_STREAM, logger, "suppressing a SOFT EXPIRE event");
			return;
		}
	}
			...
			struct logger *logger)
{
	...
	if (addr.nl_pid != 0) {
		...
		name_buf sb;
		ldbg(logger, "%s() ignoring %s message from process %u",
		     __func__, str_sparse_long(&xfrm_type_names, rsp.n.nlmsg_type, &sb),
		     addr.nl_pid);
		return true;
	}
...
static void netlink_xfrm_message_processor(struct nlm_resp *rsp, struct logger *logger)
{
	...
	ldbg(logger, "%s() got %s message with length %zu",
	     __func__,
	     str_sparse_long(&xfrm_type_names, rsp->n.nlmsg_type, &xfrmb),
	     (size_t) rsp->n.nlmsg_len);
...
static void netlink_rtm_message_processor(struct nlm_resp *rsp, struct logger *logger)
{
	...
	ldbg(logger, "%s() got %s message with length %zu",
	     __func__,
	     str_sparse_long(&rtm_type_names, rsp->n.nlmsg_type, &rtmb),
	     (size_t) rsp->n.nlmsg_len);
